...
// MenuContainer.fn API
    _e(function (E, $) {
        ...
        MenuContainer.fn._addOneMenu = function (menu) {
            ...
            if ($menuNormal.children().hasClass('wangeditor-menu-img-close')) {
                $item.css("float", "right");
                $item.css("margin-left", "14px");
            }
            ...
            $.each(configColors, function (k, v) {
                $content.append(
                    [
                        ...
                        '    title="' + v + '" commandValue="' + k + '" ',
                        '    style="color: ' + k + '" ',
                        '><i class="wangeditor-menu-img-color"></i></a>'
                    ].join('')
                );
            });
            $content.on('click', 'a[commandValue]', function (e) {
    ...
    setTimeout(function () {
        ...
        E.createMenu(function (check) {
            ...
            var menu = new E.Menu({
                ...
                $domNormal: $('<a href="#" tabindex="-1"><i class="wangeditor-menu-img-close"></i></a>'),
                $domSelected: $('<a href="#" tabindex="-1" class="selected"><i class="wangeditor-menu-img-close"></i></a>')
            });
            // 菜单正常状态下，点击将触发该事件
            menu.clickEvent = function (e) {
                var overlay = window.hdmap.map.getOverlayById(editor.keyId);
                window.hdmap.map.removeOverlay(overlay);
            };
            // 增加到editor对象中
            editor.menus[menuId] = menu;
        });
